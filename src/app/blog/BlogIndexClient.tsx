...
export default function BlogIndexClient() {
  ...
  const getCategoryLabel = (cat: string): string => {
    const key = cat as keyof typeof t.blog.categories
      ...
      <main className="max-w-5xl mx-auto px-4 py-10">
        ...
        <div className="text-center mb-12">
          <div className="inline-block px-4 py-1 rounded-full bg-neon-pink/10 text-neon-pink text-xs font-bold mb-4">
            {t.blog.badge} · {posts.length} {t.blog.articles}
          </div>
          <h1 className="h-title text-gradient mb-4">{t.blog.title}</h1>
            ...
            <article className="glass-card p-6 md:p-10 group hover:scale-[1.01] transition-transform">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-white/50 text-xs">
                  {CATEGORY_EMOJIS[featured.category]} {getCategoryLabel(featured.category)}
                </span>
                <span className="text-white/40 text-xs">· {featured.readTime} {t.blog.readTime}</span>
              </div>
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h2 className="text-2xl md:text-3xl font-bold text-white mb-3 group-hover:text-gradient">
                    {getLocalizedText(featured.title as unknown as Record<string, string>)}
                  </h2>
                  <p className="text-white/70 leading-relaxed mb-4">
                    {getLocalizedText(featured.description as unknown as Record<string, string>)}
                  </p>
              ...
              <article className="glass-card p-6 h-full hover:scale-[1.02] transition-transform">
                <div className="flex items-center gap-2 mb-3 text-xs">
                  <span className="text-neon-cyan">
                    {CATEGORY_EMOJIS[post.category]} {getCategoryLabel(post.category)}
                  </span>
                  <span className="text-white/40">· {post.readTime} {t.common.minRead}</span>
                </div>
                <div className="flex items-start gap-3 mb-3">
                  <span className="text-4xl">{post.emoji}</span>
                  <h3 className="text-lg font-bold text-white group-hover:text-gradient flex-1">
                    {getLocalizedText(post.title as unknown as Record<string, string>)}
                  </h3>
                </div>
                <p className="text-white/60 text-sm leading-relaxed mb-3 line-clamp-3">
                  {getLocalizedText(post.description as unknown as Record<string, string>)}
                </p>
        ...
        <div className="mt-16 glass-card p-8 text-center">
          <h2 className="text-2xl font-bold text-gradient mb-3">{t.blog.ctaTitle}</h2>
